class BookmarksController < ApplicationController
  ...
  def index
    ...
    @bookmarks = @q.result(:distinct => true).includes(:user, :dish, :venues).page(params[:page]).per(10)
  ...
  def create_row_from_venue
    ...
    if @bookmark.valid?
      ...
      redirect_to("/venues/#{@bookmark.venues_id}", notice: "Bookmark created successfully.")
    else
      render("bookmark_templates/new_form_with_errors.html.erb")
    end
  end
  ...
  def destroy_row_from_venues
    @bookmark = Bookmark.find(params.fetch("id_to_remove"))

    @bookmark.destroy

    redirect_to("/venues/#{@bookmark.venues_id}", notice: "Bookmark deleted successfully.")
  end
